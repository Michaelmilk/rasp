...
备注：如果需要通过网线连接，建议在cmdline.txt中一同配置默认网关。cmdline.txt的配置，会配置/etc/network/interfaces的配置，导致后者的默认网关无效。

例如要配置默认网关为192.168.100.1：
```
ip=192.168.100.100::192.168.100.1
```
...
4. `sudo apt-get install python-dev libevent-dev python-setuptools`
5. `sudo pip install gevent`，需要比较长时间
6. `sudo pip install bottle`
7. `sudo pip install gevent-socketio`
8. `sudo pip install grequests`
9. `sudo pip install spidev`

在Windows上的环境配置（如果Node、Server或者Forwarder需要在Windows上运行）：

如果仅在Windows上使用浏览器查看，则不需要执行下面的部分。
